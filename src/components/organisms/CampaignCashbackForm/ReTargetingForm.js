...
const ReTargetingForm = ({ initialValues, prev, onFinish, onPublished }) => {
  ...
  const handleCancel = () => {
    history.push('/admin/campaigns/cashbacks')
  }
  return (
          ...
          <Row gutter={16}>
            <Col span={24}>
              <Form.Item
                ...
                label={intl.get('campaigns.rewards.form.locations.rePostcode')}
              >
                <Select mode='multiple' showSearch>
                  {Array.isArray(rePostcodes) &&
              ...
              <Form.Item
                ...
                label={intl.get('campaigns.rewards.form.locations.reRetailers')}
              >
                <Select mode='multiple' showSearch>
                  {Array.isArray(reRetailers) &&
                    reRetailers.map(retailer => (
                      <Option key={retailer.participant_id} value={retailer.alias}>
                        {retailer.alias}
                      </Option>
                    ))}
                </Select>
