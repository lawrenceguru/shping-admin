...
const categoriesColumns = [
  {
    title: `${intl.get('shoppingListUsagePage.attribute.name')}`,
    dataIndex: 'name',
    key: 'name',
    render: (i, data) => {
      let name = 'Unknown'
      if (data && data.attribute && data.attribute.length > 0) {
        name = data.attribute.map(attribute => attribute.name).join(',')
      }
  ...
  {
    title: `${intl.get('shoppingListUsagePage.attribute.showCount')}`,
...
const ShoppingListTableCategory = ({ dataIndex, ...props }) => {
  ...
  return (
    <TopTable
      ...
      rowKey={() => uuid()}
      headerText={intl.get('shoppingListUsagePage.attributeTableTitle')}
      isFooter
      viewAll
      pagination
      {...props}
