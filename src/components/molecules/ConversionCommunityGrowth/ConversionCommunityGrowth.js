...
import useGetConversionCommunityGrowth from './useGetConversionCommunityGrowth'
...
const ConversionCommunityGrowth = ({
  setItem,
  selectFirstDate,
  selectSecondDate,
  selectBrand,
  ...
  const { interactions } = useGetConversionCommunityGrowth({
  ...
  return (
    <CampaignsChartWrapper {...props}>
      <div style={{ height: '100%' }} className='pdf-export' data-index={dataIndex}>
        <WidgetTemplate>
          <GraphHeader name={intl.get('conversionPage.communityGrowth')} setItem={setItem} />
          {interactionValues && interactionValues.length ? (
            <StyledChart plotOptions={plotOptions} chart={chart} chartHeight={700}>
              ...
              <YAxis>
                <ColumnSeries name={intl.get('conversionPage.communityGrowth')} data={interactionValues} />
              </YAxis>
            </StyledChart>
          ) : (
            <NoDataPlaceholder />
...
ConversionCommunityGrowth.propTypes = {
...
ConversionCommunityGrowth.defaultProps = {
  dataIndex: null
}

export default withHighcharts(ConversionCommunityGrowth, Highcharts)
